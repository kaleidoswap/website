---
title: "KaleidoAgent: An Autonomous AI Agent for Bitcoin L2 via MCP"
date: "2026-05-06"
tags: ["Deep Dive"]
slug: "kaleidoagent-wdk-hackathon"
excerpt: "KaleidoAgent executes Bitcoin and Lightning payments for AI agents via MCP servers. Trustless atomic swaps on Bitcoin L2 introduced in agentic payments."
...
**Nanobot Manager.** The central runtime, written in Node.js. It owns scheduling, LLM reasoning, the Telegram gateway, and the lifecycle of the MCP servers. Cron jobs are stored as JSON under `.nanobot/cron/jobs.json` and synced from the AI agent config.
...
- **`skill` mode (default).** SKILL.md files are loaded directly at runtime as system prompts. The AI agent issues a unified `run_kaleido_command` prompt that shells out to the `kaleido` CLI. Lightweight, predictable, easy to reason about.
...
The tradeoff is real: Bitcoin L2s are more heterogeneous than EVM. Lightning, RGB, Spark, and on-chain each have their own state machines, their own liveness assumptions, and their own failure modes. There is no single virtual machine to target, no uniform mempool, no one-size-fits-all RPC. Building an AI agent here means writing code that respects those differences instead of papering over them.

That is exactly why an AI agent is the right shape for this problem. The complexity that used to make Bitcoin L2 hard for end users, channel management, RGB transfers, atomic swap orchestration, cross-layer routing, is the kind of work software is good at. KaleidoAgent absorbs the complexity so the operator keeps the property that mattered in the first place: their bitcoin, in their custody, working for them.

KaleidoAgent does that in three concrete ways:

**Trustless swaps.** Rebalances use a 5-step HTLC atomic swap natively on the Lightning Network. The AI agent never holds counterparty risk against an exchange. The protocol scales peer-to-peer.

**Non-custodial by construction.** Every signature is produced locally by WDK. There is no remote signer, no escrow, no bridge custodian. If the AI agent disappears tomorrow, the keys, and the funds, stay with the operator.
...
Lightning is the right rails for this. It settles in seconds, it is global, it is permissionless, and it natively supports streaming and sub-cent payments. That is the shape of money the machine economy needs. Cards do not work for a service that gets paid per inference call. Bank transfers do not work for an AI agent that buys data from another AI agent. Lightning does. It is the best form of money we have for a world where most economic activity is going to be initiated by software, not by people.

The next step is **L402 and agentic payments at scale**. Your AI agent already pays for premium market data over Lightning today. The same pattern extends naturally: an AI agent that pays for compute, for storage, for APIs, for goods, on your behalf, inside the limits you set. You tell it what you want your portfolio to look like, what you want to buy, what risk you accept, and it goes and transacts. No card, no custodian, no monthly invoice.
