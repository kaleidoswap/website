import { Globe, Monitor, Smartphone, Puzzle, Download, Clock, ArrowUpRight, Terminal, ChevronRight, LockOpen, Bot, Github } from 'lucide-react'
import { motion } from 'framer-motion'
import { Button } from '@/components/common/Button'
import { PRODUCTS, DOCS, SOCIALS, GITHUB } from '@/constants/urls'
...
// Green "live" status chip with pulsing dot
const StatusChip = ({ label }: { label: string }) => (
  <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-primary-500/10 text-primary-400 border border-primary-500/20 shrink-0">
    ...
    <section className="py-16">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 auto-rows-[minmax(180px,auto)]">
            ...
            >
              ...
              <div className="p-8 relative z-10 h-full flex flex-col">
                <div className="flex items-start justify-between mb-5">
                  <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-secondary-400 to-secondary-600 flex items-center justify-center text-white shadow-lg shadow-secondary-500/30">
                    <Terminal className="w-6 h-6" />
                  </div>
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-secondary-500/10 text-secondary-400 border border-secondary-500/20">
                    <span className="w-1.5 h-1.5 rounded-full bg-secondary-400 animate-pulse shrink-0" />
                    {t('Testnet')}
                  </span>
                </div>
            ...
            <motion.div className={CARD_BASE} whileHover={CARD_HOVER} transition={SPRING}>
              ...
              <div className="p-6 relative z-10 flex flex-col h-full">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-secondary-500/10 border border-secondary-500/20 text-xs text-secondary-400 font-medium w-fit">
                    {t('Ready to Integrate')}
                  </span>
                </div>
                <h3 className="text-base font-bold text-white mb-1.5">{t('AI Tools')}</h3>
                <p className="text-sm text-slate-400 leading-relaxed">
                  {t('AI agent, local AI, MCP servers, and skills.')}
                </p>
                <div className="mt-auto pt-4 flex items-center gap-4">
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => handleNavigation(GITHUB.agentRepoUrl, true)}
                    className="gap-2"
                  >
                  ...
                  <button
                    onClick={() => handleNavigation('/products/ai-tools', false)}
                    className="flex items-center gap-1 text-sm text-slate-400 hover:text-white transition-colors"
                  >
                    {t('Learn more')}
                    <ChevronRight className="w-3.5 h-3.5" />
          ...
          {/* ── Mobile — coming soon (half, under SDK) ────────────────────── */}
          <AnimateIn variant="fade-up" delay={400} className="md:col-span-1 lg:col-span-1 h-full">
            <motion.div className={`${CARD_BASE} opacity-70 hover:opacity-100 transition-opacity duration-300`} whileHover={CARD_HOVER} transition={SPRING}>
              <div className="p-6 relative z-10 flex flex-col h-full">
                <CardIcon color="bg-primary-500/10 text-primary-400">
          ...
          {/* ── Web App — coming soon (half, under SDK) ───────────────────── */}
          <AnimateIn variant="fade-up" delay={500} className="md:col-span-1 lg:col-span-1 h-full">
            <motion.div className={`${CARD_BASE} opacity-70 hover:opacity-100 transition-opacity duration-300`} whileHover={CARD_HOVER} transition={SPRING}>
              <div className="p-6 relative z-10 flex flex-col h-full">
                <CardIcon color="bg-primary-500/10 text-primary-400">
                ...
                <p className="text-sm text-slate-400 leading-relaxed">
                  {t('Connect your wallet, no download required.')}
                </p>
                <div className="mt-auto pt-4">
                  <ComingSoonBadge label={t('Coming Soon')} />
                </div>
